...
def render_export_step(app_version: str) -> None:
    ...
    # Clear stale PDF-commit state left by older versions of the app. Export is
    # now based on the last server-saved editor state, so it must not wait for an
    # unbounded browser acknowledgement.
    clear_pdf_editor_commit_request(st.session_state)
