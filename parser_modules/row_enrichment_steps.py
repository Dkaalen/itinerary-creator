"""Focused enrichment steps for parsed itinerary rows."""
...
def prepare_main_text(row: dict, *, description: str, item_type: str, separate_city: str) -> str:
    main_text = str(description or "").strip().strip('"')
    main_text = strip_matching_type_prefix(main_text, item_type)
    main_text = _apply_pipe_city(row, main_text, separate_city)
    main_text = _apply_colon_city(row, main_text, separate_city)
    return fix_common_text_for_row(main_text, item_type)


def _apply_pipe_city(row: dict, main_text: str, separate_city: str) -> str:
    if separate_city or "|" not in main_text:
        return main_text
    ...
    if is_valid_city_value(first_pipe_part) and not re.search(r"\bnorway\s+in\s+a\s+nutshell\b", first_pipe_part, flags=re.IGNORECASE):
        ...
        return rest_pipe_text.strip()
    return main_text


def _apply_colon_city(row: dict, main_text: str, separate_city: str) -> str:
    if separate_city:
...
def apply_city_and_quality(row: dict, *, main_text: str, description: str, item_type: str, current_day: str) -> None:
    ...
    if not row.get("is_optional") and normalize_type(item_type) in _CITY_REQUIRED_TYPES and not row["city"]:
        diagnostics.warn(
            "missing_city",
...
def apply_common_detail_fields(row: dict, main_text: str) -> None:
    ...
    notable_source = extract_detail(main_text, "Notable Sights") or extract_detail(main_text, "Highlights") or extract_detail(main_text, "Stops")
...
def apply_hotel_fields(row: dict, *, main_text: str, item_type: str, start_date: str, end_date: str, night_count_hint: str) -> None:
    ...
    if date_derived_nights:
        _apply_date_derived_nights(hotel_details, date_derived_nights, main_text)
    row.update(hotel_details)
    if row.get("hotel_name"):
        row["title"] = row["hotel_name"]


def _apply_date_derived_nights(hotel_details: dict, date_derived_nights: str, main_text: str) -> None:
