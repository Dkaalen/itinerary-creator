"""Serialize calculator state for backup import/export."""
...
CALCULATOR_BACKUP_SCHEMA_VERSION = 1
...
def calculator_state_to_dict(state: CalculatorState) -> dict[str, Any]:
    """Return a versioned JSON-safe payload for calculator state."""
...
def calculator_state_from_dict(payload: Mapping[str, Any]) -> CalculatorState:
    """Build calculator state from a versioned backup payload."""
    ...
    if version != CALCULATOR_BACKUP_SCHEMA_VERSION:
        raise ValueError(f"Unsupported calculator backup schema version: {version}.")

    state = create_calculator_state(str(payload.get("itinerary_name") or ""))
