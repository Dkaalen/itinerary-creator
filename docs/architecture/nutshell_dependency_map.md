...
| Product detection | `itinerary_domain.activity_product_rules.norway`, `itinerary_generation.product_rules.find_product_match`, `itinerary_generation.transport_norway._is_norway_in_a_nutshell_text` | `activity_product.canonical_family == "norway_in_a_nutshell"` | Detection is repeated by text-marker checks outside the product rule. |
| Normalization | `normalizer_modules.transport_title.normalize_transport_title`, `normalizer_modules.transport_activity_detection.is_rail_or_fjord_route_activity`, `normalizer_modules.transport_transfer_detection.is_route_transfer_activity`, `normalizer_modules.core` | Canonical title, `effective_type`, route metadata | Product is represented as a generic row plus loosely coupled metadata. |
