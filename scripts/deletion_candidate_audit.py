...
def report_payload(
    ...
    return {"report_fingerprint": fingerprint.to_dict(), **report.to_dict()}

def write_markdown(report: DeletionCandidateReport, path: Path, *, fingerprint: ReportFingerprint | None = None) -> None:
...
def main() -> int:
    ...
    args.json_output.write_text(
        json.dumps(report_payload(report, fingerprint), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    write_markdown(report, args.markdown_output, fingerprint=fingerprint)
