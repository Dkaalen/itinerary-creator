...
def render_export_step(app_version):
    if st.session_state.itinerary_html:
        st.markdown(
            '<div class="panel-card-title">5 — Export and delivery</div>'
            '<div class="workflow-note">Download the editable project, save the HTML preview, or create the final PDF. Create PDF applies pending page and image edits first.</div>',
            unsafe_allow_html=True,
        )

        html_path = Path(st.session_state.html_path) if st.session_state.html_path else None
        pictures_added_for_status = pictures_are_added(st.session_state.get("output_edits", {}))
        export_status_html = "".join([
            metric_card_html("Project", "Ready", "Editable JSON"),
            metric_card_html("HTML", "Ready" if html_path and html_path.exists() else "Missing", "Preview file"),
            metric_card_html("Images", "Ready" if pictures_added_for_status else "Pending", "Required for PDF"),
            metric_card_html("PDF", st.session_state.get("pdf_status", "Not created"), "Final export"),
        ])
        st.markdown(f'<div class="metric-grid">{export_status_html}</div>', unsafe_allow_html=True)
