...
def analyze_schedule_occupancy(events: Iterable[object]) -> DayScheduleOccupancy:
    ...
    if not timed:
        return DayScheduleOccupancy(has_invalid_time_range=invalid)

    timed.sort(key=lambda event: int(getattr(event, "start_minutes")))
    first_start = min(int(getattr(event, "start_minutes")) for event in timed)
    last_end = max(int(getattr(event, "end_minutes")) for event in timed)
    arranged = sum(max(0, int(getattr(event, "end_minutes")) - int(getattr(event, "start_minutes"))) for event in timed)
    span = max(0, last_end - first_start)
    gaps = [
        max(0, int(getattr(second, "start_minutes")) - int(getattr(first, "end_minutes")))
    ...
    return DayScheduleOccupancy(
        ...
        has_invalid_time_range=invalid,
...
__all__ = ["DayScheduleOccupancy", "analyze_schedule_occupancy"]
