...
def _row_date_suffix(row):
    text = format_client_date(row.get("start_date"))
    return f" - {text}" if text else ""


def _commercial_row_title(row):
...
def _specific_self_arranged_items(parsed_rows):
    items = []
    for row in main_rows_only(parsed_rows or []):
        ...
        if not (is_self_arranged(row) or row.get("commercial_status") == "self_arranged" or "self transfer" in text):
            continue
        title = _commercial_row_title(row)
        if not title:
            continue
        label = f"{title}{_row_date_suffix(row)}"
        add_unique(items, label)
    return items


def _specific_optional_items(parsed_rows):
    items = []
    for row in optional_rows_only(parsed_rows or []):
        title = _commercial_row_title(row)
        if not title:
            continue
        add_unique(items, f"{title}{_row_date_suffix(row)}")
    return items
...
def create_whats_not_included(parsed_rows=None):
    ...
    self_arranged_items = _specific_self_arranged_items(rows)
    ...
    optional_items = _specific_optional_items(rows)
    if optional_items:
