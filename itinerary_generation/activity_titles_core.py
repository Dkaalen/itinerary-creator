...
from itinerary_generation.activity_title_matchers import match_keyword_activity_title
...
def create_client_activity_title(row):
    ...
    supplier_heading = _supplier_day_heading(row, details)
    if supplier_heading:
        return supplier_heading

    title = _sanitized_candidate_title(row, title, original_title, details)
    title = _fallback_title_from_details(title, details)
    if is_bad_raw_day_title(title):
        title = ""

    product_title = _activity_product_title(row, title, original_title, details)
    if product_title:
        return product_title

    title = _normalize_pre_keyword_title(title)
    keyword_title = match_keyword_activity_title(
        row,
        title=title,
    ...
    full_text = f"{title_text} {original_title.lower()} {details}".lower()
...
def _supplier_day_heading(row: dict, details: str) -> str:
    for heading_source in (row.get("original_title"), details):
        supplier_heading = _extract_supplier_day_heading(heading_source or "")
        if supplier_heading and not is_bad_raw_day_title(supplier_heading) and re.match(
            r"^\s*Day\s+\d+\s*[:\-–]",
            str(heading_source or ""),
            flags=re.IGNORECASE,
        ):
            return supplier_heading
    return ""
...
def _sanitized_candidate_title(row: dict, title: str, original_title: str, details: str) -> str:
...
def _fallback_title_from_details(title: str, details: str) -> str:
    if title:
        return title
...
def _activity_product_title(row: dict, title: str, original_title: str, details: str) -> str:
...
def _normalize_pre_keyword_title(title: str) -> str:
