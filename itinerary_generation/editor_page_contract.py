...
def document_pages_from_draft(editor_draft: Mapping[str, Any] | None) -> list[Mapping[str, Any]]:
    """Return canonical document pages from an editor draft payload."""

    if not isinstance(editor_draft, Mapping):
        return []
    raw_pages = editor_draft.get("document_pages")
...
def page_is_hidden(hidden_ids: set[str] | frozenset[str] | Sequence[str] | None, page_id: str) -> bool:
    """Return whether a page ID is hidden in a normalized hidden-page collection."""
...
def final_section_is_hidden(hidden_ids: set[str] | frozenset[str] | Sequence[str] | None, section_id: str) -> bool:
    """Return whether a final section page is hidden by its canonical page ID."""
...
def page_order_from_document_pages(document_pages: Any) -> list[str]:
    """Return page IDs in editor sort order, ignoring malformed page entries."""
...
def page_order_from_draft(editor_draft: Mapping[str, Any] | None) -> list[str]:
    """Return page IDs in editor sort order from an editor draft payload."""
...
def ordered_page_ids(default_ids: Sequence[str], requested_order: Sequence[str] | None) -> list[str]:
    """Merge an optional saved page order with the renderer's default page IDs."""

    default_page_ids = [str(page_id) for page_id in default_ids if str(page_id or "").strip()]
    ordered = [str(page_id) for page_id in (requested_order or []) if str(page_id) in default_page_ids]
    ordered.extend(page_id for page_id in default_page_ids if page_id not in ordered)
    return ordered
...
def manual_pages_from_document_pages(document_pages: Any, hidden_ids: set[str] | frozenset[str] | Sequence[str] | None = None) -> list[dict[str, Any]]:
    """Extract visible manual pages and their HTML blocks from document pages."""

    hidden = set(str(item) for item in (hidden_ids or hidden_page_ids(document_pages)))
    pages: list[dict[str, Any]] = []
    if not isinstance(document_pages, (list, tuple)):
        return pages
    for page in document_pages:
        if not isinstance(page, Mapping) or page.get("page_type") != "manual":
            continue
        page_id = str(page.get("page_id") or "").strip()
        ...
        for block in blocks:
            if not isinstance(block, Mapping):
                continue
            fields = block.get("editable_fields") if isinstance(block.get("editable_fields"), Mapping) else {}
...
def manual_pages_from_draft(editor_draft: Mapping[str, Any] | None, hidden_ids: set[str] | frozenset[str] | Sequence[str] | None = None) -> list[dict[str, Any]]:
    """Extract visible manual pages from an editor draft payload."""
