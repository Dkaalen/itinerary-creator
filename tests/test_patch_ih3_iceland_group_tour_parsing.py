...
def test_ih3_all_gts_and_gtw_sheets_integrate_one_package_with_ordered_days() -> None:
...
def test_ih3_parser_accepts_explicit_group_tour_and_commercial_row_types() -> None:
...
def test_ih3_commercial_rows_are_related_but_never_merged_into_package_days() -> None:
...
def test_ih3_explicitly_selected_commercial_upgrade_is_not_left_optional() -> None:
    sheet = _sheet("5D GTS")
...
def test_ih3_package_day_source_text_and_supplier_order_are_preserved() -> None:
    sheet = _sheet("5D GTW")
...
def test_ih3_sheet_season_metadata_wins_and_conflict_remains_visible() -> None:
...
def test_ih3_pre_and_post_tour_services_remain_independent_rows() -> None:
...
def test_ih3_real_group_tour_fixture_is_annotated_automatically() -> None:
    source = (REPO_ROOT / "tests" / "fixtures" / "real_inputs" / "iceland_group_tour_winter.txt").read_text(
        encoding="utf-8"
    )
...
def test_ih3_ordinary_guided_activities_are_not_linked_into_a_package() -> None:
...
def test_ih3_runtime_integration_does_not_import_reference_corpus() -> None:
    source = (REPO_ROOT / "itinerary_generation" / "group_tour_parsing.py").read_text(encoding="utf-8")
