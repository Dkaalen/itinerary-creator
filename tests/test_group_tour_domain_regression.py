...
def _normalized_fixture(name: str) -> list[dict]:
    fixture_text = (REPO_ROOT / "tests" / "fixtures" / "real_inputs" / name).read_text(encoding="utf-8")
    return normalize_itinerary_rows(parse_itinerary(fixture_text))


def test_all_iceland_group_sheets_build_one_ordered_package_contract() -> None:
...
def test_sheet_season_wins_without_silently_rewriting_conflicting_source() -> None:
...
def test_package_accommodation_policy_preserves_promise_without_inventing_hotels() -> None:
...
def test_commercial_rows_are_separate_from_package_days_and_inclusions() -> None:
...
def test_daily_segments_preserve_optional_and_overnight_facts() -> None:
...
def test_contract_metadata_roundtrip_is_versioned_and_lossless() -> None:
...
def test_normalized_winter_fixture_overrides_false_overview_excluded_status() -> None:
...
def test_normalized_summer_fixture_keeps_package_day_offset_and_daily_source() -> None:
...
def test_annotation_attaches_contract_only_to_master_and_package_days() -> None:
    sheet = _sheet("5D GTW")
...
def test_ordinary_activities_do_not_become_group_tour_packages() -> None:
    rows = [
        {
            "day": "Day 1",
            "type": "Activity",
...
def test_runtime_contract_does_not_import_reference_corpus() -> None:
    domain_source = read_contract_text(REPO_ROOT / "itinerary_generation" / "group_tour_domain.py")

    assert "from itinerary_generation.reference_corpus" not in domain_source
    assert "import itinerary_generation.reference_corpus" not in domain_source
