...
def test_quality_gate_blocks_price_or_currency_leak():
...
def test_group_tour_output_has_no_prices_or_raw_programme_heading_leaks():
    rows = _rows()
...
def test_optional_group_tour_extra_is_not_rendered_as_included():
    rows = _rows()
...
def test_important_notes_are_real_paragraphs_not_fragments():
    fragments = [
        "Transport schedules",
        "including flights",
        "trains",
...
def test_default_only_bank_can_return_auto_fallback_images(tmp_path):
...
def test_preview_image_contract_contains_bank_status(tmp_path):
