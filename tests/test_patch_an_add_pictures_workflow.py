...
def test_add_pictures_button_activates_pictures_without_browser_commit_handshake():
    ...
    assert 'set_pictures_added(st.session_state.output_edits, True)' in handler
    assert 'rebuild_current_preview(mark_pdf_dirty=True, force=True, save_html=True)' in handler
    assert 'st.session_state["_visual_editor_commit_nonce"] = None' in handler
