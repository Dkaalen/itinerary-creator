...
def test_requests_use_the_same_primary_city_as_day_image_matching():
...
def test_downloads_only_requested_destination_packs_and_reuses_cache(monkeypatch, tmp_path):
...
def test_checksum_failure_never_activates_corrupt_pack(monkeypatch, tmp_path):
    oslo = _pack_bytes("Norway", "Oslo")
...
def test_safe_extraction_rejects_traversal(monkeypatch, tmp_path):
    stream = BytesIO()
...
def test_connection_adds_missing_pack_without_full_repo_clone(monkeypatch, tmp_path):
    oslo = _pack_bytes("Norway", "Oslo")
    bergen = _pack_bytes("Norway", "Bergen")
    entries = {
        "Norway/Oslo": _manifest_entry("Norway", "Oslo", oslo),
...
def test_refreshes_stale_cached_manifest_before_full_repo_fallback(monkeypatch, tmp_path):
    oslo = _pack_bytes("Norway", "Oslo")
