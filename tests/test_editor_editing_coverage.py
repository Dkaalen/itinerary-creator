...
def test_frontend_exposes_remaining_static_labels_as_editable_fields():
    source = _frontend_source()

    assert "cover.route_label" in source
    assert "summary.trip_glance_title" in source
    assert "summary.journey_arc_title" in source
    assert "summary.journey_arc_columns.chapter" in source
    assert "summary.journey_arc_columns.days" in source
    assert "summary.journey_arc_columns.experience" in source
    assert "days.${i}.date" in source
    assert "final_pages.${titleKey" in source
    assert "data-empty-label" in source
    assert ".table-header-edit" in source
