...
def test_bz1f_obsolete_compatibility_files_stay_deleted() -> None:
    for relative in REMOVED_PATHS:
        assert not (ROOT / relative).exists(), f"obsolete file returned: {relative}"


def test_bz1f_no_static_or_dynamic_production_references_remain() -> None:
    offenders: list[str] = []
...
def test_bz1f_artifact_policy_rejects_browser_cache_and_runtime_outputs() -> None:
    forbidden = (
...
def test_bz1f_production_import_smoke_has_no_non_optional_failures(monkeypatch) -> None:
...
def test_bz1f_clean_zip_survives_fresh_extraction(tmp_path) -> None:
