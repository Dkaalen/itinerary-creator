...
def test_activity_contract_preserves_guide_and_snow_condition() -> None:
    rows, grouped = _state()
...
def test_icebreaker_keeps_both_timezones_and_meal_exclusion() -> None:
    rows, grouped = _state()
...
def test_snowhotel_stay_inclusions_reach_day_and_final_pages() -> None:
    rows, grouped = _state()
    render_document = build_render_document(rows, grouped)
...
def test_day_headers_use_the_day_start_date_only() -> None:
    rows, grouped = _state()
...
def test_day_train_direction_service_and_provisional_note_are_canonical() -> None:
    rows, grouped = _state()
...
def test_day_8_keeps_all_three_logistics_across_consumers() -> None:
    rows, grouped = _state()
    render_document = build_render_document(rows, grouped)
...
def test_final_inclusions_and_day_blocks_share_source_facts() -> None:
    rows, grouped = _state()
...
def test_exact_pdf_has_no_blank_overflow_page_and_keeps_client_facts(tmp_path) -> None:
    import pytest
