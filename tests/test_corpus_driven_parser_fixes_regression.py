...
def test_city_column_survives_when_details_cell_is_blank():
...
def test_product_prefix_is_not_misread_as_city():
    raw = _row(
        "Day 3",
        "Activity",
        "Tromso",
...
def test_activity_prose_title_is_shortened_before_duration_body_text():
    raw = _row(
        "Day 2",
        "Activity",
        "Rovaniemi",
...
def test_activity_city_can_be_inferred_from_title_when_source_city_is_blank():
    raw = _row(
        "Day 2",
        "Activity",
        "",
...
def test_shared_supplier_suffix_does_not_become_title():
    raw = _row(
        "Day 5",
        "Activity",
        "Kakslauttanen",
...
def test_decimal_nights_are_not_misread_as_city_prefix():
    raw = _row(
        "Day 9",
        "Hotel",
        "",
...
def test_sparse_departure_uses_same_day_city_context():
    raw = "\n".join([
...
def test_sparse_transfer_before_hotel_is_backfilled_from_same_day_city():
    raw = "\n".join([
...
def test_sparse_activity_uses_recent_city_context():
    raw = "\n".join([
...
def test_blank_fixed_excel_details_do_not_turn_dates_into_titles():
...
def test_meeting_point_city_can_supply_sparse_activity_city():
    raw = _row(
        "Day 5",
        "Activity",
        "",
...
def test_sparse_departure_summary_does_not_require_route_points():
...
def test_norway_in_a_nutshell_route_points_are_extracted():
    raw = _row(
        "Day 2",
        "Activity",
        "Voss",
...
def test_timed_multileg_transport_route_points_are_extracted():
    raw = _row(
        "Day 7",
        "Transfer",
        "Trondheim",
...
def test_local_transfer_summaries_do_not_require_route_points():
    raw = _row(
        "Day 1",
        "Transfer",
        "Reykjavik",
...
def test_cost_row_after_transfer_does_not_pollute_transfer_title():
    raw = "\n".join([
...
def test_nutshell_schedule_without_dashes_extracts_first_and_last_place():
    raw = _row(
        "Day 3",
        "Activity",
        "Gudvangen",
...
def test_timed_airport_flight_extracts_route_points():
    raw = _row(
        "Day 4",
        "Transfer",
        "Oslo",
...
def test_self_arranged_flight_without_route_points_is_not_review_noise():
    raw = _row(
        "Day 4",
        "Transfer",
        "Oslo",
