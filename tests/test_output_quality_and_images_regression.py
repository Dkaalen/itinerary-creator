...
def test_real_fixture_journey_arc_uses_destination_welcome_not_travel_filler():
    rows = _fixture_rows()
...
def test_image_bank_path_discovery_is_side_effect_free(monkeypatch, tmp_path):
...
def test_image_bank_bootstrap_can_be_disabled(monkeypatch, tmp_path):
    root = tmp_path / "itinerary-creator-git"
    fallback = root / "image_bank"
    fallback.mkdir(parents=True)
...
def test_default_images_lose_when_destination_bank_has_matches(tmp_path):
