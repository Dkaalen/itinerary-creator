...
def test_image_bank_status_identifies_full_destination_bank(tmp_path):
...
def test_default_only_bank_is_blocking_status(tmp_path):
...
def test_audit_warns_about_default_only_bank_before_pdf_export(tmp_path):
    bank = tmp_path / "image_bank"
...
def test_client_quality_gate_warns_about_missing_full_image_bank(tmp_path):
    bank = tmp_path / "image_bank"
    _write_webp(bank / "Default" / "Default_Autumn_City_01.webp")
    status = image_bank.image_bank_status_for_paths([bank])
...
def test_image_path_lookup_does_not_clone_runtime_repo(monkeypatch, tmp_path):
...
def test_explicit_runtime_setup_fetches_image_bank(monkeypatch, tmp_path):
    root = tmp_path / "itinerary-creator-git"
    fallback = root / "image_bank"
    fallback.mkdir(parents=True)
...
def test_selected_image_audit_contains_country_region_score(tmp_path):
    bank = tmp_path / "image_bank_full"
    _write_webp(bank / "Norway" / "Oslo" / "Oslo_Autumn_City_01.webp")
