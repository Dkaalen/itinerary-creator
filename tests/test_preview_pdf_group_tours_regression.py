...
def test_group_tour_days_are_not_misclassified_as_leisure():
...
def test_group_tour_route_uses_programme_cities_not_flybus_service_label():
    rows = _group_tour_rows()
...
def test_preview_image_contract_preserves_reviewed_picture_data_uri(tmp_path):
...
def test_typed_pdf_uses_preview_contract_data_uri_when_path_is_unavailable(tmp_path):
