...
def test_real_product_identity_route_and_commercial_status_are_preserved() -> None:
    rows = _real_rows()
...
def test_real_product_keeps_adjacent_transfers_as_separate_ordered_rows() -> None:
...
def test_supplier_inclusions_keep_compound_leg_order() -> None:
...
def test_oslo_to_bergen_timetable_preserves_order_modes_and_times() -> None:
...
def test_partial_route_preserves_known_points_without_inventing_times() -> None:
    source = (
...
def test_commercial_status_survives_product_normalization(
    row_type: str,
    suffix: str,
    expected_status: str,
    expected_reason: str,
...
def test_day_renderer_uses_canonical_nutshell_title() -> None:
    grouped = group_rows_by_day(_real_rows())
