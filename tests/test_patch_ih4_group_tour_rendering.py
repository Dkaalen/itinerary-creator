...
def test_ih4_all_ten_sheets_render_one_canonical_block_per_package_day() -> None:
...
def test_ih4_day_header_block_and_editor_share_the_same_contract() -> None:
    for name in GROUP_SHEETS:
...
def test_ih4_final_inclusions_list_each_package_once_not_each_package_day() -> None:
    for name in GROUP_SHEETS:
...
def test_ih4_package_accommodation_does_not_replace_pre_or_post_tour_hotels() -> None:
...
def test_ih4_legacy_package_accommodation_is_not_recreated_as_hotel_products() -> None:
...
def test_ih4_optional_and_commercial_items_never_become_package_inclusions() -> None:
...
def test_ih4_preview_contains_one_package_and_ordered_daily_segments() -> None:
...
def test_ih4_typed_pdf_uses_the_same_summer_and_winter_contract(name: str, tmp_path: Path) -> None:
...
def test_ih4_route_summary_and_season_are_package_owned() -> None:
...
def test_ih4_season_conflict_remains_visible_without_cross_contaminating_rendering() -> None:
...
def test_ih4_client_output_quality_accepts_all_ten_group_tour_sheets() -> None:
    for name in GROUP_SHEETS:
